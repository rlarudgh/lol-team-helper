...
import { calculateTeamBalance } from "@/shared/utils/team-balance.utils";
...
export function TeamStrengthIndicator({
  redTeam,
  blueTeam,
}: TeamStrengthIndicatorProps) {
  ...
  const strongerTeam =
    redBalance > blueBalance
      ? "red"
      : blueBalance > redBalance
      ? "blue"
      : "balanced";

  const getStrengthLevel = (diff: number) => {
    if (diff < 100)
      return { level: "균형", color: "bg-green-500", icon: Minus };
    if (diff < 300)
      return { level: "약간 우세", color: "bg-yellow-500", icon: TrendingUp };
    if (diff < 600)
      return { level: "우세", color: "bg-orange-500", icon: TrendingUp };
